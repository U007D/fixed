...
impl<F: Fixed> Saturating<F> {
    ...
    ///
    /// use fixed::{types::I16F16, Saturating};
    /// assert_eq!(Saturating::<I16F16>::INT_BITS, I16F16::INT_BITS);
    /// ```
    ...
    ///
    /// use fixed::{types::I16F16, Saturating};
    /// assert_eq!(Saturating::<I16F16>::FRAC_BITS, I16F16::FRAC_BITS);
    /// ```
...
macro_rules! op {
    ($Trait:ident, $saturating:ident, $Op:ident $op:ident, $OpAssign:ident $op_assign:ident) => {
        impl<F: $Trait> $Op<Saturating<F>> for Saturating<F> {
        ...
        impl<F: $Trait> $OpAssign<Saturating<F>> for Saturating<F> {
            #[inline]
        ...
        impl<F: $Trait> $OpAssign<F> for Saturating<F> {
            #[inline]
...
//
//     impl<const FRAC: i32> Op<i8> for Saturating<FixedI8<FRAC>> { /* ... */ }
//     impl<const FRAC: i32> Op<&i8> for Saturating<FixedI8<FRAC>> { /* ... */ }
//     impl<const FRAC: i32> Op<i16> for Saturating<FixedI16<FRAC>> { /* ... */ }
//     impl<const FRAC: i32> Op<&i16> for Saturating<FixedI16<FRAC>> { /* ... */ }
//     ...

macro_rules! op_bits {
    (
        $Fixed:ident($Bits:ident $(, $nbits:expr)?)::$saturating:ident,
        $Op:ident $op:ident,
        $OpAssign:ident $op_assign:ident
    ) => {
        impl<const FRAC: i32> $Op<$Bits> for Saturating<$Fixed<FRAC>>
        ...
        impl<const FRAC: i32> $OpAssign<$Bits> for Saturating<$Fixed<FRAC>>
            $(where If<{ (0 <= FRAC) & (FRAC <= $nbits) }>: True)?
        {
            #[inline]
...
macro_rules! ops {
    ($Fixed:ident($Bits:ident, $nbits:expr)) => {
        ...
        op_bits! { $Fixed($Bits, $nbits)::rem, Rem rem, RemAssign rem_assign }
    };
}
ops! { FixedI8(i8, 8) }
ops! { FixedI16(i16, 16) }
